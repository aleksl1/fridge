...
const FoodDiaryListItem: FunctionComponent<FoodDiaryListItemProps> = (itemProps) => {
    const {macrosPerPiece: {fats, proteins, carbs}, quantity, name} = itemProps.item
    return <View>
        <DataTable.Row onPress={() => {
        }}>
            <DataTable.Cell style={{flex: 2}}>{quantity} of {name}</DataTable.Cell>
            <DataTable.Cell numeric>{calculateCaloriesFromMacros({proteins, fats, carbs})}</DataTable.Cell>
