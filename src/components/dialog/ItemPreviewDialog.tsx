...
const ItemPreviewDialog: FunctionComponent<ItemPreviewDialogProps> = ({
  ...
  const caloriesInfo = () => (
    <>
      <List.Item
        title={`calories in one piece: ${calculateCaloriesFromMacros({
          fats,
          carbs,
          proteins,
        })}`}
      ...
      <View style={{ marginStart: spacing.spacing16 }}>
        <List.Item title={`proteins: ${proteins}`} left={listLeftIcon} />
        <List.Item title={`fats: ${fats}`} left={listLeftIcon} />
        <List.Item title={`carbs: ${carbs}`} left={listLeftIcon} />
      </View>
    </>
  );
